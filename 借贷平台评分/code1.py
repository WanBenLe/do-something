...
data0 = pd.read_csv('data0.csv', encoding='ANSI')
data0['年龄'] = data0['年龄'].astype('str')
data0['逾期次数'] = data0['逾期次数'].astype('float')
data0 = data0.values

data1 = pd.read_csv('data1.csv', encoding='ANSI')
data1['年龄'] = data1['年龄'].astype('str')
data1['逾期次数'] = data1['逾期次数'].astype('float')
data1 = data1.values

data2 = pd.read_csv('data2.csv', encoding='ANSI')
data2['年龄'] = data2['年龄'].astype('str')
data2['逾期次数'] = data2['逾期次数'].astype('float')
data2 = data2.values

data3 = pd.read_csv('data3.csv', encoding='ANSI')
data3['年龄'] = data3['年龄'].astype('str')
data3['逾期次数'] = data3['逾期次数'].astype('float')

data3 = data3.values

data = np.vstack((data0, data1, data2, data3))
data0 = pd.DataFrame(data, columns=['逾期次数', '标的总额', '年利率', '期限', '信用评级', '年龄', '性别', '学历 ', '婚姻',
                                    '收入'])
data1 = data0.copy()
data0.sample(frac=1)

len_all = int(len(data0['逾期次数']) * 0.7)
class_all = data0.iloc[:, 4:].values
num_all = data0.iloc[:, 0:4].values
...
dcore = np.hstack((woow.reshape(-1, 1), ypred.reshape(-1, 1), data1.values.reshape(-1, 10)))
df_r = pd.DataFrame(dcore)
df_r.to_csv('score1.csv', index=False, encoding='utf_8_sig')
print('finish')
'''
