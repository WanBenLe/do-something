...
data = pd.read_csv('truedefault.csv').values
...
def f1(x):
    ...
    for i in range(lenx):
        ...
        if sol > 0.15:
            sol = 0.15
            continue
        lirun += x[i] * (sol - data[i, 3]) * (1 - x[i + lenx])
    s = -lirun
...
def f2(x):
    ...
    for i in range(lenx):
        ...
        lirun.append(x[i] * (sol - data[i, 3]) * (1 - x[i + lenx]))
    s = np.var(lirun)
    return s
...
pd.DataFrame(data, columns=['企业代号', '信誉评级', '是否违约', '违约率', '贷款金额', '流失率', '利率']).to_csv('Q1result.csv', index=False,
                                                                                        encoding='ansi')
